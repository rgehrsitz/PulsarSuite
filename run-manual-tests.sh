#!/bin/bash
# Run manual tests with fixed test scenarios
...
echo "===== Manual Tests ====="
echo "Running BeaconTester with manually created test scenarios"
...
# Using existing test scenarios
echo "===== Using Manual Test Scenarios ====="
echo "Using manual test scenarios: $OUTPUT_DIR/generated_tests.json"
...
# Run the manually created tests
echo ""
echo "===== Running Tests ====="
cd "$PROJECT_ROOT"  # Return to project root before running tests

echo "Running BeaconTester with the manual test scenarios..."
...
if [ $TEST_RESULT -eq 0 ]; then
    ...
    echo ""
    echo "🎉 SUCCESS! 🎉"
    echo "The manually created tests ran successfully."
else
    echo "Manual tests FAILED with exit code $TEST_RESULT"
    echo ""
...
echo ""
echo "Output files:"
echo "  $OUTPUT_DIR/generated_tests.json - Manual test scenarios"
echo "  $OUTPUT_DIR/results.json - Test results"
