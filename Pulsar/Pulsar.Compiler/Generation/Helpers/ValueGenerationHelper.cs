...
namespace Common.Testing
{
    ...
    public static class ValueGenerationHelper
    {
        ...
        {
            // Assumes condition has Threshold (double) and Operator (string)
        ...
        {
            // If it's a ComparisonCondition, handle accordingly
            ...
            // If it's a ThresholdOverTimeCondition, delegate
        ...
        {
            // For simplicity, just use the first condition for now
            if (conditions.Count > 0)
                return GenerateValueForCondition(conditions[0], target);
            return GenerateDefaultValue("unknown");
        }
